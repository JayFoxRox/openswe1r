...
DECLARE_INTERFACE_(IDirectInputDeviceA, IUnknown)
{
    ...
    STDMETHOD(GetProperty)(THIS_ REFGUID,LPDIPROPHEADER) PURE; // 5
    ...
    STDMETHOD(Initialize)(THIS_ HINSTANCE,DWORD,REFGUID) PURE; // 17

    // Note: This is normally another type, but for OpenSWE1R, these are merged, as version 2 inherits the first one
    /*** IDirectInputDevice2A methods ***/
    STDMETHOD(CreateEffect)(THIS_ REFGUID,LPCDIEFFECT,LPDIRECTINPUTEFFECT *,LPUNKNOWN) PURE; // 18
    STDMETHOD(EnumEffects)(THIS_ LPDIENUMEFFECTSCALLBACKA,LPVOID,DWORD) PURE; // 19
    STDMETHOD(GetEffectInfo)(THIS_ LPDIEFFECTINFOA,REFGUID) PURE; // 20
    STDMETHOD(GetForceFeedbackState)(THIS_ LPDWORD) PURE; // 21
    STDMETHOD(SendForceFeedbackCommand)(THIS_ DWORD) PURE; // 22
    STDMETHOD(EnumCreatedEffectObjects)(THIS_ LPDIENUMCREATEDEFFECTOBJECTSCALLBACK,LPVOID,DWORD) PURE; // 23
    STDMETHOD(Escape)(THIS_ LPDIEFFESCAPE) PURE; // 24
    STDMETHOD(Poll)(THIS) PURE; // 25
    STDMETHOD(SendDeviceData)(THIS_ DWORD,LPDIDEVICEOBJECTDATA,LPDWORD,DWORD) PURE; // 26
};
